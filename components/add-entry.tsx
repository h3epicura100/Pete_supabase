        ...
        <DialogContent className="max-h-[88vh] flex flex-col p-0 max-w-2xl sm:max-w-3xl rounded-2xl sm:rounded-3xl overflow-hidden bg-white shadow-2xl z-[100]" aria-describedby={undefined}>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden min-h-0">
            <div className="p-4 sm:p-6 overflow-y-auto flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div className="space-y-1"><Label>Person</Label><Select value={formData.personName} onValueChange={val => handleSelectChange("personName", val)} disabled={currentUser.role !== "admin"}><SelectTrigger className="rounded-xl h-11"><SelectValue /></SelectTrigger><SelectContent>{dropdownOptions.personName.map(n => <SelectItem key={n} value={n}>{n}</SelectItem>)}</SelectContent></Select></div>
              ...
              <div className="space-y-1"><Label>Mode</Label><Select value={formData.mode} onValueChange={val => handleSelectChange("mode", val)}><SelectTrigger className="rounded-xl h-11"><SelectValue placeholder="Mode" /></SelectTrigger><SelectContent>{dropdownOptions.mode.map(m => <SelectItem key={m} value={m}>{m}</SelectItem>)}</SelectContent></Select></div>
              <div className="space-y-1"><Label>Group Head</Label><div className="flex gap-2"><Select value={formData.groupHead} onValueChange={val => handleSelectChange("groupHead", val)}><SelectTrigger className="flex-1 rounded-xl h-11"><SelectValue /></SelectTrigger><SelectContent>{dropdownOptions.groupHead.map(g => <SelectItem key={g} value={g}>{g}</SelectItem>)}</SelectContent></Select><Button type="button" size="icon" variant="outline" onClick={() => setIsGroupHeadModalOpen(true)} className="rounded-xl h-11 w-11 shrink-0"><PlusCircle className="h-4 w-4" /></Button></div></div>
              <div className="space-y-1 sm:col-span-2"><Label>Reason</Label><div className="flex gap-2"><Select value={formData.reason} onValueChange={val => handleSelectChange("reason", val)}><SelectTrigger className="flex-1 rounded-xl h-11"><SelectValue /></SelectTrigger><SelectContent>{dropdownOptions.reason.map(r => <SelectItem key={r} value={r}>{r}</SelectItem>)}</SelectContent></Select><Button type="button" size="icon" variant="outline" onClick={() => setIsReasonModalOpen(true)} className="rounded-xl h-11 w-11 shrink-0"><PlusCircle className="h-4 w-4" /></Button></div></div>
