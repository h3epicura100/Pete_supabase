    ...
    <div className="p-2 space-y-6">
      <Card className="border-[#ede9fe] shadow-xl shadow-slate-200/50 rounded-2xl overflow-hidden">
        <CardHeader className="bg-violet-50/70 border-b border-violet-100/80">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Button onClick={() => setIsFormOpen(true)} className="bg-gradient-to-r from-violet-600 to-fuchsia-500 text-white font-bold h-11 rounded-xl shadow-lg shadow-violet-500/20">
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="max-h-[85vh] overflow-y-auto max-w-4xl rounded-3xl" aria-describedby={undefined}>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 pb-4">
            ...
            <div className="space-y-1">
              ...
              <div className="flex gap-2">
                ...
                <Button type="button" size="icon" variant="outline" onClick={() => setIsVendorModalOpen(true)} className="rounded-xl h-11 w-11"><PlusCircle className="h-4 w-4" /></Button>
              </div>
            </div>
            <div className="space-y-1">
            ...
            <div className="md:col-span-2 space-y-1">
              <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Attachment (Optional)</Label>
            ...
            <div className="md:col-span-2 flex justify-end gap-3 pt-6">
              <Button type="button" variant="outline" onClick={() => setIsFormOpen(false)} className="rounded-xl px-6">Cancel</Button>
              <Button type="submit" disabled={isSubmitting} className="bg-violet-600 text-white font-bold px-8 rounded-xl shadow-lg shadow-violet-200">{isSubmitting ? "Processing..." : "Add Entry"}</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isVendorModalOpen} onOpenChange={setIsVendorModalOpen}>
        <DialogContent aria-describedby={undefined}>
          ...
          <div className="py-2 space-y-2"><Label>Vendor Name</Label><Input value={newVendorName} onChange={e => setNewVendorName(e.target.value)} placeholder="Name" className="rounded-xl" /></div>
          <DialogFooter><Button variant="outline" onClick={() => setIsVendorModalOpen(false)} className="rounded-xl">Cancel</Button><Button onClick={handleAddNewVendor} disabled={isAddingVendor} className="rounded-xl">Add Vendor</Button></DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="max-w-3xl rounded-2xl p-6 bg-white border border-slate-100 shadow-xl" aria-describedby={undefined}>
          <DialogHeader className="flex flex-row items-center justify-between pb-3 border-b border-slate-100 space-y-0">
            <DialogTitle className="text-slate-800 font-bold text-base flex items-center gap-2">
              ...
              <span>{isPdfFile(previewImageUrl) ? "Receiving Invoice PDF Document" : "Receiving Invoice Attachment"}</span>
            </DialogTitle>
      ...
      <Card className="border-[#ede9fe] shadow-xl shadow-violet-500/5 rounded-2xl overflow-hidden mt-6 p-6 space-y-4">
        <CardHeader className="bg-violet-50/70 border border-violet-100/80 rounded-xl px-4 py-3">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <CardTitle className="text-slate-800 text-sm md:text-base font-bold flex items-center gap-2"><Filter className="h-4 w-4 text-violet-500" /> History ({visibleRecords.length})</CardTitle>
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button className="bg-[#7c3aed] text-white font-bold h-10 rounded-xl px-4 flex items-center gap-2 shadow-lg shadow-violet-100">
                    ...
                    <div className="space-y-3">
                      <div className="space-y-1">
                        ...
                        <div className="relative">
                            ...
                            <Input type="date" value={tempDateRange.from} onChange={e => { const v = e.target.value; setTempDateRange(p => { const u = { ...p, from: v }; setDateRange({ from: u.from ? new Date(u.from) : undefined, to: u.to ? new Date(u.to) : undefined }); return u }) }} className="pl-8 h-9 rounded-xl text-xs" />
                        </div>
                      </div>
                      <div className="space-y-1">
                        ...
                        <div className="relative">
                            <CalendarIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-3 w-3 text-slate-300" />
                            <Input type="date" value={tempDateRange.to} onChange={e => { const v = e.target.value; setTempDateRange(p => { const u = { ...p, to: v }; setDateRange({ from: u.from ? new Date(u.from) : undefined, to: u.to ? new Date(u.to) : undefined }); return u }) }} className="pl-8 h-9 rounded-xl text-xs" />
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="relative w-full lg:w-64">
                ...
                <Input value={tableSearch} onChange={e => setTableSearch(e.target.value)} placeholder="Search..." className="pl-9 rounded-xl h-10" />
              </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-violet-50/80 sticky top-0 backdrop-blur-sm z-10">
                <tr className="border-b border-violet-100">
                  <th className="p-4 text-left text-[10px] font-black uppercase tracking-widest text-violet-600">Timestamp</th>
                  <th className="p-4 text-left text-[10px] font-black uppercase tracking-widest text-violet-600">Entry Date</th>
                  <th className="p-4 text-left text-[10px] font-black uppercase tracking-widest text-violet-600">Vendor</th>
                  <th className="p-4 text-left text-[10px] font-black uppercase tracking-widest text-violet-600">Invoice #</th>
                  <th className="p-4 text-right text-[10px] font-black uppercase tracking-widest text-violet-600">Amount</th>
                ...
                {isRecordsLoading ? (
                  ...
                  visibleRecords.map(rec => (
                    <tr key={rec.id} className="border-b border-slate-100 hover:bg-slate-50/80 transition-colors">
                      <td className="p-4 text-slate-500 text-xs">{formatDisplayTimestamp(rec.timestamp)}</td>
                      <td className="p-4 text-slate-600 font-medium">{formatEntryDate(rec.date)}</td>
                      <td className="p-4 font-bold text-slate-800">{rec.vendorName}</td>
                      <td className="p-4 text-slate-600">{rec.invoiceNumber}</td>
                      <td className="p-4 text-right font-mono font-bold text-violet-700">₹{rec.invoiceAmt.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                      <td className="p-4"><span className="inline-flex px-2 py-1 rounded-md bg-slate-100 text-[10px] font-bold uppercase tracking-wider text-slate-500">{rec.mode}</span></td>
